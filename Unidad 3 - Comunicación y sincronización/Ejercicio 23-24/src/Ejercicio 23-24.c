 ...
 */
 ...
 void init_shared_resource() {
	 ...
	 fprintf(stdout, "Memoria compartida es creada con fd: %d\n", shared_fd);
 ...
 *counter = 0;
 ...
 if (pid) { // The parent process
	 ...
	 inc_counter();
     fprintf(stdout, "El proceso padre ve el contador = %d.\n", *counter);
     int status = -1;
     wait(&status);
     fprintf(stdout, "El proceso hijo termino con status = %d.\n", status);

 }

 else { // The child process
     inc_counter();
     fprintf(stdout, "El proceso hijo ve el contador = %d.\n", *counter);
 }
